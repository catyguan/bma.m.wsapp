 ...
 * generated in one exchange. It provides methods for examining the request from
 ...
 * reading the request body. After reading the request body, the stream is
 * close.
 * <li>{@link #getResponseHeaders()} to set any response headers, except
 * content-length
 * <li>{@link #sendResponseHeaders(int,long)} to send the response headers. Must
 * be called before next step.
 * <li>{@link #getResponseBody()} to get a {@link java.io.OutputStream} to send
 * the response body. When the response body has been written, the stream must
 * be closed to terminate the exchange.
 * </ol>
 * <b>Terminating exchanges</b> <br>
 * Exchanges are terminated when both the request InputStream and response
 * OutputStream are closed. Closing the OutputStream, implicitly closes the
 * InputStream (if it is not already closed). However, it is recommended to
 * consume all the data from the InputStream before closing it. The convenience
 * method {@link #close()} does all of these tasks. Closing an exchange without
 * consuming all of the request body is not an error but may make the underlying
 * TCP connection unusable for following exchanges. The effect of failing to
 * terminate an exchange is undefined, but will typically result in resources
 * failing to be freed/reused.
 * 
 * @since 1.6
 */
...
public abstract class HttpExchange {
	 ...
	 * {@link #sendResponseHeaders(int,long)}) must be called prior to calling
	 * this method. Multiple calls to this method (for the same exchange) will
	 * return the same stream. In order to correctly terminate each exchange,
	 * the output stream must be closed, even if no response body is being sent.
	 * <p>
	 * Closing this stream implicitly closes the InputStream returned from
	 * {@link #getRequestBody()} (if it is not already closed).
	 * <P>
	 * If the call to sendResponseHeaders() specified a fixed response body
	 * length, then the exact number of bytes specified in that call must be
	 * written to this stream. If too many bytes are written, then write() will
	 * throw an IOException. If too few bytes are written then the stream
	 * close() will throw an IOException. In both cases, the exchange is aborted
	 * and the underlying TCP connection closed.
	 * 
	 ...
	 * response length parameter is greater than zero, this specifies an exact
	 * number of bytes to send and the application must send that exact amount
	 * of data. If the response length parameter is <code>zero</code>, then
	 ...
	 * OutputStream. If response length has the value <code>-1</code> then no
	 * response body is being sent.
	 * <p>
	 * If the content-length response header has not already been set then this
	 * is set to the apropriate value depending on the response length
	 * parameter.
	 * <p>
	 ...
	 *            if > 0, specifies a fixed response body length and that exact
	 *            number of bytes must be written to the stream acquired from
	 *            getResponseBody(), or else if equal to 0, then chunked
	 *            encoding is used, and an arbitrary number of bytes may be
	 *            written. if <= -1, then no response body length is specified
	 *            and no response body may be written.
	 * @see HttpExchange#getResponseBody()
	 */
	public abstract void sendResponseHeaders(int rCode, long responseLength)
			throws IOException;
	 ...
	 * 
	 * @return the response code, if available. <code>-1</code> if not available
	 *         yet.
	 */
	...
	/**
	 * Filter modules may store arbitrary objects with HttpExchange instances as
	 * an out-of-band communication mechanism. Other Filters or the exchange
	 * handler may then access these objects.
	 * <p>
	 ...
	 * @param value
	 *            the object to store as the attribute value. <code>null</code>
	 *            value is permitted.
	 * @throws NullPointerException
	 *             if name is <code>null</code>
	 */
	 ...
	 * this call must wrap the original streams, and may be (but are not
	 * required to be) sub-classes of {@link java.io.FilterInputStream} and
