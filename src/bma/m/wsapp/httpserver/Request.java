...
class Request {
	...
	/**
	 * read a line from the stream returning as a String. Not used for reading
	 * headers.
	 */
	...
	static class ReadStream extends InputStream {
		...
		public ReadStream(ServerImpl server, SocketChannel chan)
				throws IOException {
			this.channel = chan;
			this.server = server;
		...
		public synchronized int read(byte[] b, int off, int srclen)
				throws IOException {
	...
	static class WriteStream extends java.io.OutputStream {
		...
		public WriteStream(ServerImpl server, SocketChannel channel)
				throws IOException {
