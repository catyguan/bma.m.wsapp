 ...
 * implements the interface {@link java.util.Map}&lt; {@link java.lang.String},
 * {@link java.util.List}&lt;{@link java.lang.String}&gt;&gt;. The keys are
 * case-insensitive Strings representing the header names and the value
 * associated with each key is a {@link List}&lt;{@link String}&gt; with one
 * element for each occurence of the header name in the request or response.
 * <p>
 * For example, if a response header instance contains one key "HeaderName" with
 * two values "value1 and value2" then this object is output as two header
 * lines: <blockquote>
 * 
 * <pre>
 * HeaderName: value1
 ...
 * <li>{@link #add(String,String)} adds the given header value to the list for
 * the given key</li>
 ...
 * values. However, null keys will never will be present in HTTP request
 * headers, and will not be output/sent in response headers. Null values can be
 * represented as either a null entry for the key (i.e. the list is null) or
 * where the key has a list, but one (or more) of the list's values is null.
 * Null values are output as a header line containing the key but no associated
 * value.
 * 
 * @since 1.6
 */
public class Headers implements Map<String, List<String>> {
	...
	/*
	 * Normalize the key by converting to following form. First char upper case,
	 * rest lower case. key is presumed to be ASCII
	 */
